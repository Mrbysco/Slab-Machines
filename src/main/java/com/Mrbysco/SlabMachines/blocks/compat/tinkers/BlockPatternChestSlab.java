...
public class BlockPatternChestSlab extends BlockSlab implements ITinkerStationBlock {
	public static final PropertyEnum<BlockPatternChestSlab.Variant> VARIANT = PropertyEnum.<BlockPatternChestSlab.Variant>create("variant", BlockPatternChestSlab.Variant.class);
	public static final PropertyDirection FACING = new PropertyUnlistedDirection("facing", EnumFacing.Plane.HORIZONTAL);

    public BlockPatternChestSlab() {
		super(Material.WOOD);
		
		this.setUnlocalizedName(SlabReference.MOD_PREFIX + "pattern_chest_slab".replaceAll("_", ""));
		this.setRegistryName("pattern_chest_slab");
		this.setHardness(2.0F);
		this.setResistance(5.0F);
	    this.setSoundType(SoundType.WOOD);
		useNeighborBrightness = true;
		setCreativeTab(SlabMachines.slabTab);
		setDefaultState(blockState.getBaseState().withProperty(VARIANT, BlockPatternChestSlab.Variant.DEFAULT).withProperty(HALF, EnumBlockHalf.BOTTOM).withProperty(FACING, EnumFacing.NORTH));
	...
	public Comparable<?> getTypeForItem(ItemStack stack) {
        return BlockPatternChestSlab.Variant.DEFAULT;
	}
	...
	public void onBlockPlacedBy(World worldIn, BlockPos pos, IBlockState state, EntityLivingBase placer,
		...
		TilePatternChestSlab te = SlabUtil.getTileSlab(worldIn, pos, state.getValue(HALF), TilePatternChestSlab.class);
	...
	public boolean openGui(EntityPlayer player, World world, BlockPos pos) {
		int id = world.getBlockState(pos).getValue(HALF) == EnumBlockHalf.TOP ? SlabGuiHandler.GUI_PATTERNCHEST_SLAB_TOP : SlabGuiHandler.GUI_PATTERNCHEST_SLAB_BOTTOM;
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
	    ...
	    if(!heldItem.isEmpty() && te != null && te instanceof TileTinkerChest) {
	        ...
	        if(rest.isEmpty() || rest.getCount() < heldItem.getCount()) {
	        	playerIn.inventory.mainInventory.set(playerIn.inventory.currentItem, rest);
	          return true;
	        }
	    }
	...
	public TileEntity createTileEntity(World world, IBlockState state) {
        return new TilePatternChestSlab();
	}
	...
	public boolean rotateBlock(World world, BlockPos pos, EnumFacing axis) {
		TilePatternChestSlab table = SlabUtil.getTileSlab(world, pos, world.getBlockState(pos).getValue(HALF), TilePatternChestSlab.class);
	    if(table != null) {
	...
	private void writeDataOntoItemstack(@Nonnull ItemStack item, @Nonnull IBlockAccess world, @Nonnull BlockPos pos, @Nonnull IBlockState state, boolean inventorySave) {
		...
		TilePatternChestSlab te = SlabUtil.getTileSlab(world, pos, state.getValue(HALF), TilePatternChestSlab.class);
	    if(te != null) {
